...
for index, row in df.iterrows():
    ...
    ranking_objective = energy + pmv + eval(dgp_var) + eval(udi_var)
    ...
    if column1_ind == column1_obj:
        success_rate_df.loc[index, 'obj_success_1'] = 1
    if column1_ind != column1_obj:
        success_rate_df.loc[index, 'obj_success_1'] = 0

    if column1_ind == column1_clu:
        success_rate_df.loc[index, 'clu_success_1'] = 1
    if column1_ind != column1_clu:
        success_rate_df.loc[index, 'clu_success_1'] = 0

    if column2_ind == column2_obj:
        success_rate_df.loc[index, 'obj_success_2'] = 1
    if column2_ind != column2_obj:
        success_rate_df.loc[index, 'obj_success_2'] = 0

    if column2_ind == column2_clu:
        success_rate_df.loc[index, 'clu_success_2'] = 1
    if column2_ind != column2_clu:
        success_rate_df.loc[index, 'clu_success_2'] = 0

    if column3_ind == column3_obj:
        success_rate_df.loc[index, 'obj_success_3'] = 1
    if column3_ind != column3_obj:
        success_rate_df.loc[index, 'obj_success_3'] = 0

    if column3_ind == column3_clu:
        success_rate_df.loc[index, 'clu_success_3'] = 1
    if column3_ind != column3_clu:
        success_rate_df.loc[index, 'clu_success_3'] = 0

    success_rate_df.loc[index, 'archetype'] = occ_clu


print(success_rate_df)

# Save the dataframe used for clustering (non-scaled)
success_rate_df.to_csv('C:/Users/prana/OneDrive - Delft University of Technology/Thesis/'
                       '02_Working/Excel/02_Qualtrics Data/success.csv')
...
plot_1.set_xlabel('Shade preference distribution by performance', fontsize=20)
plot_1.set_ylabel('Count', fontsize=20)
plot_1.set_ylim(0, 130)
...
plot_1.set_xticklabels(plot_1.get_xticklabels(), rotation=45)
update_plot(plot_1, 20)
plot_1.set_xlabel('Shade preference distribution by performance and individual weights', fontsize=20)
plot_1.set_ylabel('Count', fontsize=20)
plot_1.set_ylim(0, 130)
# Add count labels on top of each bar
for p in plot_1.patches:
